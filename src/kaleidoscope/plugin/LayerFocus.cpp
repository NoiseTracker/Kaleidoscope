...
EventHandlerResult LayerFocus::onFocusEvent(const char *command) {
  ...
  if (strcmp_P(command + 6, "activate")) {
    if (!::Focus.isEOL()) {
      uint8_t layer;
      ::Focus.read(layer);
      ::Layer.activate(layer);
    }
  } else if (strcmp_P(command + 6, "deactivate")) {
    if (!::Focus.isEOL()) {
      uint8_t layer;
      ::Focus.read(layer);
      ::Layer.deactivate(layer);
    }
  } else if (strcmp_P(command + 6, "isActive")) {
    if (!::Focus.isEOL()) {
      uint8_t layer;
      ::Focus.read(layer);
      ::Focus.send(::Layer.isActive(layer));
    }
  } else if (strcmp_P(command + 6, "moveTo")) {
    if (!::Focus.isEOL()) {
      uint8_t layer;
      ::Focus.read(layer);
      ::Layer.move(layer);
    }
  } else if (strcmp_P(command + 6, "state")) {
    if (::Focus.isEOL()) {
      Runtime.serialPort().println(Layer.getState(), BIN);
    } else {
      Layer.move(0);
      Layer.deactivate(0);
